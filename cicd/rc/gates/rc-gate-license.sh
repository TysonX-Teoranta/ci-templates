...
#          space-separated SPDX ids; permissive default); RC_LICENSE_PACKAGE_MAP
#          (optional reviewed Name@Version=SPDX mappings for packages whose NuGet
#          metadata does not expose a machine-readable licence).
set -uo pipefail
...
PACKAGE_MAP="${RC_LICENSE_PACKAGE_MAP:-}"
for mapping in $PACKAGE_MAP; do
  case "$mapping" in *@*=*) ;; *) die "license: invalid package mapping '$mapping' (expected Name@Version=SPDX)" 2 ;; esac
done
...
BAD="$(jq -r --arg allow "$ALLOW" --arg package_map "$PACKAGE_MAP" '
  ...
  | (.components // [])[]
  | . as $c
  | "\($c.name)@\($c.version // "?")" as $package_key
  | ( ( .licenses // [] )
  ...
  | . as $l
  | select(
      ($ok | index($l | ascii_upcase)) == null
      and (($mapped[$package_key] // "") | ascii_upcase) as $declared
      | ($ok | index($declared)) == null
    )
  | "\($c.name) \($c.version // "?") — \($l)"
